...
namespace VisionMaster.Scada.Controls
{
    ...
    public sealed class ElementPropertyDescriptor : IPropertySpec
    {
        ...
        ///
        /// <b>这不是随手填的默认值，是一条产品口径</b>：<b>设计期版面语言不可绑，运行期动画语言可绑</b>。
        /// 所以尺寸（液位涨落）与角度（指针 / 阀门开度）可绑，而位置（<c>$X</c>/<c>$Y</c>）<b>不可绑</b>——
        /// 让画面每次刷新都自己搬家，操作员会以为界面出了故障。逐条判据见
        /// <see cref="GeometryProperties.Standard"/> 的注释。
        /// </summary>
