...
namespace HslCommunication.Core.Address
{
	...
	public class McAddressData : DeviceAddressDataBase
	{
		...
		{
			OperateResult<McAddressData> operateResult = ParseMelsecFrom(address, length, isBit: false);
		...
		public static OperateResult<McAddressData> ParseMelsecFrom(string address, ushort length, bool isBit)
		{
			...
			{
				return new OperateResult<McAddressData>(DeviceAddressDataBase.GetUnsupportedAddressInfo(address, ex));
			}
		...
		private static int CalculateComplexAddress(string address)
		{
			...
			if (address.IndexOf(".") < 0)
			{
				...
				return Convert.ToInt32(address.Substring(0, address.Length - 2)) * 16 + Convert.ToInt32(address.Substring(address.Length - 2), 10);
			}
