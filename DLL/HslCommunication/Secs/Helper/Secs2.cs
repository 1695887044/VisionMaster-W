	...
	public class Secs2
	{
		...
		internal static void AddCodeAndValueSource(List<byte> bytes, SecsValue value, Encoding encoding)
		{
			if (value != null)
			{
				...
				{
					int length = (value.Value as IEnumerable<SecsValue>)?.Count() ?? 0;
					AddCodeSource(bytes, value.ItemType, length);
					return;
				}
				byte[] array = null;
				array = value.ItemType switch
				{
					...
					SecsItemType.Bool => (!(value.Value.GetType() == typeof(bool))) ? ((bool[])value.Value).Select((bool m) => (byte)(m ? byte.MaxValue : 0)).ToArray() : ((!(bool)value.Value) ? new byte[1] : new byte[1] { 255 }), 
