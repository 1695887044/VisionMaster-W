...
namespace HslCommunication.Profinet.Omron
{
	...
	public class OmronFinsNetHelper
	{
		...
		public static OperateResult Write(IOmronFins omron, string address, bool[] values)
		{
			...
			OperateResult<byte[]> operateResult = BuildWriteWordCommand(omron.PlcType, address, values.Select((bool m) => (byte)(m ? 1 : 0)).ToArray(), isBit: true);
			if (!operateResult.IsSuccess)
			{
				return operateResult;
			}
		...
		public static async Task<OperateResult> WriteAsync(IOmronFins omron, string address, bool[] values)
		{
			...
			OperateResult<byte[]> command = BuildWriteWordCommand(omron.PlcType, address, values.Select((bool m) => (byte)(m ? 1 : 0)).ToArray(), isBit: true);
			if (!command.IsSuccess)
			{
				return command;
			}
