...
namespace HslCommunication.Profinet.AllenBradley
{
	...
	public class AllenBradleyNet : DeviceTcpNet, IReadWriteCip, IReadWriteNet
	{
			...
			try
			{
				...
				if (num == 83 || num == 82)
				{
					...
					for (int i = 0; i < list.Count; i++)
					{
						...
						num2 += list[i].Length;
						byte[] array3 = (list[i] = AllenBradleyHelper.PackCommandSpecificData(new byte[4], PackCommandService(PortSlot ?? new byte[2] { 1, b }, array)));
					}
					return OperateResult.CreateSuccessResult(list);
				}
				byte[] array4 = AllenBradleyHelper.PackRequestWrite(address, typeCode, data, length);
				byte[] item = AllenBradleyHelper.PackCommandSpecificData(new byte[4], PackCommandService(PortSlot ?? new byte[2] { 1, b }, array4));
		...
		public override OperateResult Write(string address, bool[] value)
		{
			return WriteTag(address, 193, value.Select((bool m) => (byte)(m ? 1 : 0)).ToArray(), (!HslHelper.IsAddressEndWithIndex(address)) ? 1 : value.Length);
		}
		...
		public override async Task<OperateResult> WriteAsync(string address, string value, Encoding encoding)
		{
			...
			OperateResult write = await WriteAsync(address + ".LEN", data.Length).ConfigureAwait(continueOnCapturedContext: false);
			if (!write.IsSuccess)
			{
				return write;
			}
		...
		public override async Task<OperateResult> WriteAsync(string address, bool[] value)
		{
			return await WriteTagAsync(address, 193, value.Select((bool m) => (byte)(m ? 1 : 0)).ToArray(), (!HslHelper.IsAddressEndWithIndex(address)) ? 1 : value.Length);
		}
