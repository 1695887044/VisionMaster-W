...
namespace HslCommunication.Enthernet.Redis
{
	...
	public class RedisClient : NetworkDoubleBase
	{
		...
		{
			base.LogNet?.WriteDebug(ToString(), StringResources.Language.Send + " : " + (LogMsgFormatBinary ? send.ToHexString(' ') : Encoding.UTF8.GetString(send)));
			...
			if (operateResult2.IsSuccess)
			{
				base.LogNet?.WriteDebug(ToString(), StringResources.Language.Receive + " : " + (LogMsgFormatBinary ? operateResult2.Content.ToHexString(' ') : Encoding.UTF8.GetString(operateResult2.Content)));
			}
		...
		public override async Task<OperateResult<byte[]>> ReadFromCoreServerAsync(Socket socket, byte[] send, bool hasResponseData = true, bool usePackHeader = true)
		{
			...
			if (read.IsSuccess)
			{
				base.LogNet?.WriteDebug(ToString(), StringResources.Language.Receive + " : " + (LogMsgFormatBinary ? read.Content.ToHexString(' ') : Encoding.UTF8.GetString(read.Content)));
			}
			return read;
		}
		...
		private OperateResult<string[]> OperateArrayFromServer(string[] commands)
		{
			...
			if (@string.StartsWith(":"))
			{
				return OperateResult.CreateSuccessResult(new string[1] { @string.TrimEnd('\r', '\n').Substring(1) });
			}
			if (@string.StartsWith("+"))
			{
				return OperateResult.CreateSuccessResult(new string[1] { @string.Substring(1).TrimEnd('\r', '\n') });
			}
			if (@string.StartsWith("-MOVED"))
			{
				RedisClient cluster = getCluster(@string.TrimEnd('\r', '\n').Split(new char[1] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[2]);
				if (cluster == null)
				{
					return new OperateResult<string[]>(@string);
				}
